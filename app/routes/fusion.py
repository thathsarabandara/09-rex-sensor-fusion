...
async def verify_robot_access(robot_id: str, user=Depends(get_current_user)):
    ...
    if not await ownership_service.verify_ownership(user_id, robot_id):
        raise HTTPException(
            ...
            detail={
                "success": False,
                "error": {"code": "ROBOT_NOT_OWNED", "message": "User does not own this robot"},
            },
        )
    return user_id
    ...
    if not state:
        raise HTTPException(
            ...
            detail={
                "success": False,
                "error": {
                    "code": "FUSED_STATE_NOT_FOUND",
                    "message": "Fused state is not currently available",
                },
            },
        )
...
async def get_fusion_events(
    ...
    query = (
        select(FusionEvent)
        .where(FusionEvent.robot_id == robot_id)
        .order_by(FusionEvent.occurred_at.desc())
    )
    if event_type:
