...
def apply_ema_filter(current_val: float, previous_ema: Optional[float], alpha: float = settings.ULTRASONIC_EMA_ALPHA) -> float:
...
def filter_ultrasonic_distance(
    ...
    if not (settings.ULTRASONIC_MIN_DISTANCE_CM <= raw_distance <= settings.ULTRASONIC_MAX_DISTANCE_CM):
