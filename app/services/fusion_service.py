...
from app.schemas.fused_state import (
    FusedState,
    ImpactState,
    LineStateOut,
    MotionState,
...
class FusionService:
    ...
    async def process_snapshot(self, snapshot: SensorSnapshot):
        ...
        if snapshot.sequence < latest_seq:
            ...
            if latest_seq - snapshot.sequence > 1000:
                ...
                logger.debug(
                    f"Stale sequence {snapshot.sequence} for {robot_id}, latest is {latest_seq}"
                )
                return
        ...
        if snapshot.sensors and snapshot.sensors.ultrasonic:
            ...
            prev_ema = (
                prev_state_dict["obstacle"]["distance_cm"]
                if prev_state_dict
                and prev_state_dict.get("obstacle", {}).get("direction") == direction
                else None
            )

            filtered_d, new_samples = ultrasonic.filter_ultrasonic_distance(
                raw_d, samples, prev_ema
            )
            ...
            if filtered_d is not None:
                ...
                if prev_state_dict:
                    ...
                    if risk != "NONE" and prev_risk == "NONE":
                        await event_service.publish_and_store_event(
                            robot_id, "OBSTACLE_DETECTED", "WARNING", 0.9, {"distance": filtered_d}
                        )
                    elif risk == "NONE" and prev_risk != "NONE":
                        await event_service.publish_and_store_event(
                            robot_id, "OBSTACLE_CLEARED", "INFO", 0.9, {}
                        )
        ...
        if snapshot.sensors and snapshot.sensors.imu:
            ...
            prev_ax = (
                prev_state_dict["_internal"]["ax"]
                if prev_state_dict and "_internal" in prev_state_dict
                else None
            )
            prev_ay = (
                prev_state_dict["_internal"]["ay"]
                if prev_state_dict and "_internal" in prev_state_dict
                else None
            )
            prev_az = (
                prev_state_dict["_internal"]["az"]
                if prev_state_dict and "_internal" in prev_state_dict
                else None
            )
            ...
            if (
                unsafe_tilt
                and prev_state_dict
                and not prev_state_dict.get("tilt", {}).get("unsafe")
            ):
                await event_service.publish_and_store_event(
                    robot_id, "UNSAFE_TILT", "CRITICAL", 0.9, {}
                )
            ...
            if impact and prev_state_dict and not prev_state_dict.get("impact", {}).get("possible"):
                await event_service.publish_and_store_event(
                    robot_id, "POSSIBLE_IMPACT", "CRITICAL", 0.85, {}
                )
        ...
        if snapshot.sensors and snapshot.sensors.line:
            ...
            line_state = LineStateOut(
                detected=pos != "LOST", position=pos, normalized_error=err, confidence=0.9
            )
            ...
            if prev_state_dict:
                ...
                if pos == "LOST" and prev_pos != "LOST":
                    await event_service.publish_and_store_event(
                        robot_id, "LINE_LOST", "WARNING", 0.9, {}
                    )
                elif pos != "LOST" and prev_pos == "LOST":
                    await event_service.publish_and_store_event(
                        robot_id, "LINE_RECOVERED", "INFO", 0.9, {}
                    )
        ...
        if snapshot.robot_state:
            ...
            m_state = motion.get_motion_state(
                rs.commanded_direction, rs.commanded_speed, rs.motor_active, is_moving_accel
            )
            possible_stall = motion.detect_possible_stall(
                rs.commanded_speed, rs.motor_active, is_moving_accel
            )

            motion_state = MotionState(state=m_state, confidence=0.8, possible_stall=possible_stall)

            if (
                possible_stall
                and prev_state_dict
                and not prev_state_dict.get("motion", {}).get("possible_stall")
            ):
                await event_service.publish_and_store_event(
                    robot_id, "POSSIBLE_STALL", "WARNING", 0.8, {}
                )
        ...
        state_dict["_internal"] = {
            "ax": snapshot.sensors.imu.acceleration_x
            if snapshot.sensors and snapshot.sensors.imu
            else 0,
            "ay": snapshot.sensors.imu.acceleration_y
            if snapshot.sensors and snapshot.sensors.imu
            else 0,
            "az": snapshot.sensors.imu.acceleration_z
            if snapshot.sensors and snapshot.sensors.imu
            else 0,
        }
        ...
        if robot_id not in self.last_publish or (now - self.last_publish[robot_id]) > (
            1.0 / settings.FUSION_OUTPUT_RATE_HZ
        ):
    ...
    def _default_obstacle(self):
        return ObstacleState(
            detected=False, direction="FRONT", distance_cm=0.0, risk_level="NONE", confidence=0.0
        )
