...
def test_line_sensor():
    assert line_sensor.majority_vote([True, True, False]) == True
    assert line_sensor.majority_vote([False, False, True]) == False
    
    pos = line_sensor.get_line_position({"left_outer": True, "left_inner": True, "right_inner": False, "right_outer": False})
    assert pos == "LEFT"
