...
def test_possible_stall():
    assert motion.detect_possible_stall(50, True, False) == True
    assert motion.detect_possible_stall(50, True, True) == False

def test_orientation():
    assert orientation.is_tilt_unsafe(45, 0) == True
    assert orientation.is_tilt_unsafe(0, 45) == True
    assert orientation.is_tilt_unsafe(10, 10) == False

def test_impact():
    assert orientation.detect_possible_impact(20.0, 0.0, 0.0) == True
    assert orientation.detect_possible_impact(0.0, 0.0, 9.8) == False
